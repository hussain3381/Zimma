...
import {
  rowToProvider,
  isProviderBookable,
  UNVERIFIED_BOOK_MESSAGE,
} from "@/components/zimma/provider-mapping";
  ...
  head: () => ({
    meta: [
      { title: "Book a Service — Zimma" },
      {
        name: "description",
        content:
          "Book a verified household service professional in Karachi in 6 simple steps.",
      },
    ],
  }),
...
function toIsoDateTime(
  dateLabel: string,
  timeLabel: string,
  dates: { iso: string; weekday: string; date: string }[],
): string {
  ...
  const local = new Date(
    `${iso}T${String(h).padStart(2, "0")}:${String(min).padStart(2, "0")}:00`,
  );
...
function BookingFlow() {
  ...
  const canNextLocked = [!!date, !!time, address.line.length > 3, true];
    ...
    return (
      <div className="min-h-screen">
        ...
        <div className="mx-auto max-w-2xl px-4 py-20 text-center sm:px-6">
          ...
          <h1 className="animate-fade-up text-3xl font-bold sm:text-4xl">
            Booking confirmed!
          </h1>
          <p
          ...
          >
            Your <strong>{service?.name}</strong> with{" "}
            <strong>{provider?.name}</strong> is scheduled for {date} at {time}.
          </p>

          <Card
            className="mt-8 animate-fade-up rounded-2xl border-border bg-card p-6 text-left shadow-card"
            ...
            <div className="flex items-center gap-4">
              {provider && (
                <img
                  src={provider.avatar}
                  alt=""
                  className="h-14 w-14 rounded-xl ring-2 ring-primary/10"
                />
              )}
              <div className="flex-1">
                <p className="font-semibold">{provider?.name}</p>
                <p className="text-sm text-muted-foreground">
                  {provider?.trade} · {provider?.area}
                </p>
              </div>
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            <Link to="/dashboard/customer">
              <Button size="lg" className="shadow-glow btn-glow">
                Go to Dashboard
              </Button>
            </Link>
            <Link to="/">
      ...
      <div className="min-h-screen">
        ...
        <div className="mx-auto max-w-2xl px-4 py-20 text-center sm:px-6">
          ...
          <h1 className="text-3xl font-bold sm:text-4xl">
            This pro isn't accepting bookings yet
          </h1>
    ...
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_360px]">
          <Card
            key={step}
            className="animate-fade-up rounded-3xl border-border bg-card p-6 shadow-soft sm:p-8"
          >
            {STEPS[step] === "Service" && (
              <>
                ...
                <p className="text-sm text-muted-foreground">
                  Pick the service category — we'll match you with the best pros
                  in your area.
                </p>
            ...
            {STEPS[step] === "Provider" && (
              <>
                ...
                <p className="text-sm text-muted-foreground">
                  Fully-verified pros ready to accept bookings.
                </p>
                {providers === null ? (
                  ...
                  <p className="mt-6 rounded-2xl border border-dashed border-border bg-muted/40 p-6 text-center text-sm text-muted-foreground">
                    No fully-verified pros available yet — check back soon.
                  </p>
                ) : (
            ...
            {STEPS[step] === "Date" && (
              <>
                ...
                <p className="text-sm text-muted-foreground">
                  Choose when you'd like the pro to arrive.
                </p>
            ...
            {STEPS[step] === "Time" && (
              <>
                ...
                <p className="text-sm text-muted-foreground">
                  All times are PKT — your pro arrives within a 30-minute
                  window.
                </p>
            ...
            {STEPS[step] === "Address" && (
              <>
                <h2 className="text-lg font-bold">
                  Where should we send your pro?
                </h2>
                <p className="text-sm text-muted-foreground">
                  Provide a complete address so the pro can reach you on time.
                </p>
            ...
            {STEPS[step] === "Confirm" && (
              <>
                ...
                <div className="mt-6 space-y-3 rounded-2xl border border-border bg-muted/40 p-5">
                  ...
                  <Row
                    k="Address"
                    v={`${address.label} — ${address.line}, ${address.area}`}
                  />
                ...
                <div className="mt-4 flex items-start gap-3 rounded-2xl border border-success/30 bg-success-soft p-4 text-sm text-success">
                  ...
                  <p>
                    Free cancellation up to 1 hour before. You'll only be
                    charged after the job is completed.
                  </p>
                </div>
              </>
            )}
            ...
            <div className="mt-8 flex flex-wrap items-center justify-between gap-3 border-t border-border pt-6">
              <Button
                variant="ghost"
                onClick={() =>
                  step === 0 ? navigate({ to: "/" }) : setStep(step - 1)
                }
                className="gap-2"
              >
          ...
          <aside className="space-y-4">
            <Card className="rounded-3xl border-border bg-card p-6 shadow-card">
              ...
              <div className="mt-4 space-y-3 text-sm">
                <SummaryRow
                  icon={Sparkles}
                  k="Service"
                  v={service?.name ?? "Not selected"}
                />
                <SummaryRow
                  icon={ShieldCheck}
                  k="Pro"
                  v={provider?.name ?? "Not selected"}
                />
                ...
                <SummaryRow
                  icon={Home}
                  k="Address"
                  v={address.line ? `${address.line}, ${address.area}` : "—"}
                />
              </div>
            ...
            <Card className="rounded-3xl border-border bg-linear-to-br from-primary via-blue-600 to-blue-800 p-6 text-primary-foreground shadow-glow">
              <ShieldCheck className="h-6 w-6" />
              <p className="mt-3 text-sm font-semibold">100% Zimma Guarantee</p>
              <p className="mt-1 text-xs opacity-90">
                If anything goes wrong, we re-do the job — free of charge.
              </p>
            </Card>
          </aside>
        </div>
...
function Row({
  ...
  return (
    <div className="flex items-start justify-between gap-4 text-sm">
      ...
      <span
        className={`text-right ${bold ? "text-lg font-bold" : "font-semibold"}`}
      >
        {v}
        {sub && (
